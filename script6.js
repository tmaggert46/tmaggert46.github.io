...
// Function to calculate the final character stat
function calculateFinalCharacterStat(character) {
    const stats = characterData[character];
    const initial = initialStats[character];
    const finalStats = stats.map(stat => {
        const initialStat = initial[stat.name];
        const level = initial.Level;
        const finalStat = initialStat + (40 - level) * (stat.value / 100);
        return {
            name: stat.name,
            value: Math.round(finalStat)
        };
    });
    return finalStats;
}

// Function to calculate the fitness between two sets of stats
function calculateFitness(characterStat, classStat) {
    return characterStat.reduce((totalDistance, charStat) => {
        const classValue = classStat.find(stat => stat.name === charStat.name);
        if (classValue) {
            const predictedY = charStat.value; // Assuming character value
            const actualY = classValue.value; // Class value
            const distance = Math.abs(predictedY - actualY); // Fitness measure
            return totalDistance + distance;
        }
        return totalDistance;
    }, 0) / characterStat.length;
}

function calculateTotalStatSum(characterStat) {
  return characterStat.reduce((sum, stat) => sum + stat.value, 0);
}


// Function to update the fitness values in the table
function updateFitness() {
  for (let i = 0; i < 10; i++) {
      const characterSelect = d3.select(`#character-select-${i}`);
      const classSelect = d3.select(`#class-select-${i}`);
      
      const character = characterSelect.property("value");
      const selectedClass = classSelect.property("value");
      
      if (character && selectedClass) {
          const characterStats = calculateFinalCharacterStat(character);
          const classStats = classData[selectedClass];
          const fitness = calculateFitness(characterStats, classStats);
          const totalStatSum = calculateTotalStatSum(characterStats);

          d3.select(`#fitness-${i}`).text(fitness.toFixed(2));
          d3.select(`#total-stat-sum-${i}`).text(totalStatSum); // Add this line to display the total stat sum
      } else {
          d3.select(`#fitness-${i}`).text("");
          d3.select(`#total-stat-sum-${i}`).text(""); // Clear total stat sum if no selection
      }
  }
}
...
function initializeDropdowns() {
  ...
  for (let i = 0; i < 10; i++) {
      ...
      row.append("select")
          .attr("id", `character-select-${i}`)
          .attr("class", "character-select")
          .on("change", updateFitness);

      row.append("select")
          .attr("id", `class-select-${i}`)
          .attr("class", "class-select")
          .on("change", updateFitness);

      row.append("span")
          .attr("id", `fitness-${i}`)
          .attr("class", "fitness-score")
          .style("min-width", "45px");;

      row.append("span")
          .attr("id", `total-stat-sum-${i}`) // Add this line to create an element for total stat sum
          .attr("class", "total-stat-sum");
  }
  
  // Populate dropdowns
  Object.keys(characterData).forEach(character => {
      d3.selectAll(".character-select")
          .append("option")
